...
clientName <- 'Client Shrestha'
clientAddress <- 'area 51'
clientEmail <- 'client@area51.com'
contractorName <- 'Niranjan Shrestha'
contractorAddress <- 'NOT HOME'
contractorEmail <- 'me@area51.com'
startsDate <- '12/12/2018'
endsDate <- '12/30/2018'
finalPrice <- 750
perTime <- 5
whatDoing <- 'what are you doing sections'
billingPeriod <- 'billing period sections'
```


# Service Contract
# Monthly (Package)
# Social media marketing, SEO services, web consulting

This contract (the "Agreement") is entered into by and between the below named parties ("the Parties")
The offer will expire at the close of business on `r expireDate` if not accepted in writing by counter-signing this Agreement by the aforementioned date.

Client (The "Cient") \hfill Contractor(The "Contractor")

`r clientName` \hfill `r contractorName`

`r clientAddress` \hfill `r contractorAddress`

`r clientEmail` \hfill `r contractorEmail`  
...
**Client (`r clientName`)** \hfill 
**Contractor (`r contractorName`)**  

\vskip 0.3in

\underline{\hspace{5cm}} \underline{\hspace{2cm}} \hspace{2cm} \underline{\hspace{5cm}} \underline{\hspace{2cm}}   
Signature \hspace{3.5cm} Date \hspace{3.2cm} Signature \hspace{3.5cm} Date
...
\begin{flushright}
\textbf{\$`r finalPrice` x 1 (Fixed Fee)} \\
\textbf{`r finalPrice/perTime` x `r perTime`} \\
Total: \$`r finalPrice`
\end{flushright}  
...
## Payment Terms
\begin{flushleft}
Clients understands the importance of paying independent contractors in a timely manner and wants to maintain a positive working relationship with Contractor to keep the project moving forward.  
...
Contractor's acceptance of such service charges does not waive its rights to any remedies for Client's breach of this Agreement.  
...
\begin{flushleft}
Client shall reimburse all expenses that are reasonable and that have been authorized in writing by Client in advance; payable
within 15 days of itemized invoice.
\end{flushleft}
